    ...
    private fun print(
        ...
        object : Thread() {
            override fun run() {
                ...
                var itext: String = text

                if (itext.contains("<img>")) {
                    val images = itext.split("<img>")
                    for (img: String in images) {
                        if (img.contains("</img>")) {
                            val path: String = img.split("</img>")[0]
                            val bitmap = BitmapFactory.decodeFile(path)
                            val hexa: String = PrinterTextParserImg.bitmapToHexadecimalString(printer, bitmap)
                            itext = itext.replace(path, hexa)
                        }
                    }
                }

                printer.printFormattedTextAndCut(itext)
